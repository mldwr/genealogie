...
export default function AccountClient() {
  ...
  // Check for redirect message from /change-password
  const redirectMessage = searchParams.get('message');
  const [showRedirectInfo, setShowRedirectInfo] = useState(redirectMessage === 'password-change-redirect');
  ...
  const handlePasswordChange = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (updateError) {
        ...
        // Password changed successfully - invalidate all other sessions for security
        // This prevents continued access from other devices after password change
        try {
          await supabase.auth.signOut({ scope: 'others' });
        } catch (signOutError) {
          // Log but don't fail - password was still changed successfully
  ...
  return (
    <div className="max-w-2xl mx-auto p-4 mt-28">
      ...
      <div className="bg-white rounded-lg border border-gray-200 p-6">
        ...
        {/* Info message when redirected from /change-password */}
        {showRedirectInfo && (
          <div className="mb-4 p-3 bg-blue-50 border border-blue-200 text-blue-700 rounded-lg flex items-start">
            <svg className="w-5 h-5 mr-2 mt-0.5 text-blue-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <span className="font-medium">Sie sind bereits angemeldet.</span>
              <p className="mt-1 text-sm">Bitte verwenden Sie das folgende Formular, um Ihr Passwort zu ändern. Geben Sie dabei Ihr aktuelles Passwort zur Bestätigung ein.</p>
            </div>
            <button
              type="button"
              onClick={() => setShowRedirectInfo(false)}
              className="ml-auto -mr-1 -mt-1 p-1 text-blue-500 hover:text-blue-700"
        ...
        {passwordSuccess && (
          <div className="mb-4 p-3 bg-green-50 border border-green-200 text-green-700 rounded-lg flex items-start">
            <svg className="w-5 h-5 mr-2 mt-0.5 text-green-600 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" />
            </svg>
            <div>
              <span className="font-medium">Passwort erfolgreich geändert.</span>
              <p className="mt-1 text-sm">Ihr Passwort wurde aktualisiert. Alle anderen aktiven Sitzungen wurden aus Sicherheitsgründen abgemeldet.</p>
            </div>
          </div>
        )}
